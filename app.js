...
let data=null, rangeDays=1, selectedPlayer="all";
...
function renderSummary(){
  ...
  const fields=["total_points","economy_points","research_points","military_points"];
  const labels=["Gesamt","Ökonomie","Forschung","Militär"];
...
function render(){
  ...
  renderSummary();renderCards();renderTable();renderChart();
}
...
fetch("data.json",{cache:"no-store"}).then(r=>{
  ...
  data.players.forEach(p=>select.insertAdjacentHTML("beforeend",`<option value="${p.id}">${p.name}</option>`));
  render();
}).catch(err=>{
